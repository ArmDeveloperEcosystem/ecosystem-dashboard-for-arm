...
CAPABILITIES = {
    ...
    "database": (
        ("database", "data store"),
        ("database", "data warehouse", "data store", "data structure store"),
    ),
    "DNS serving": (
...
def capability_groups(query):
    ...
    if "database" in groups and any(
        g in groups
        for g in ("vector search", "relational database", "document database")
    ):
...
STOP |= words(
    "should trying way let good project toward towards team through across several systems system send implement implements implementing handle handles handling store stores storing saved save"
)


def stems(value):
    """Small inflection normalizer, not a package-name or semantic lookup table."""
    out = set()
    for word in words(normal(value)):
        if word.endswith("ies") and len(word) > 5:
            word = word[:-3] + "y"
        elif word.endswith("ing") and len(word) > 6:
            word = word[:-3]
        elif word.endswith("ally") and len(word) > 7:
            word = word[:-4]
            ...
            word.endswith("s")
            and len(word) > 4
            and not word.endswith(("ss", "us", "ous"))
        ):
            word = word[:-1]
        out.add(word)
    return out
...
def positive_occurrences(text, phrase):
    ...
    for match in re.finditer(pattern, text):
        ...
        if not re.search(
            r"(?:\bnon[- ]|\b(?:no|not|without)\s+(?:\w+\s+){0,2})$", prefix
        ):
            yield match
...
def database_role(package):
    ...
    if re.search(
        ...
        return False
    pattern = r"\b(?:is|are|provides|implements|offers|delivers|as)\b([^.;]{0,170}?)\b(?:database|data warehouse|data store|data structure store)\b"
    for match in re.finditer(pattern, text):
...
def role_allowed(package, group):
    text = normal(package["description"])
    opening = text[:180]
    if group in ("database", "relational database", "document database"):
    ...
    if group in ("web serving", "load balancing"):
        return has_positive(text, CAPABILITIES[group][1]) and not any(
            t in opening
...
def verified_attributes(text, attributes):
    return all(
        ...
        or (len(phrases) == 1 and bool(stems(phrases[0]) & positive_stems(text)))
...
def requested_catalog_roles(subject):
    ...
    return stems(head) & {"library", "toolkit", "driver", "compiler"}
