...
from poc.discovery.http import CollectionError, github_pages, BoundedHTTP
from poc.discovery.pipeline import review_evidence
from poc.discovery.sources import github_collect, dockerhub_collect
...
def test_ai_response_stream_enforces_total_deadline(monkeypatch):
    ...
    ticks = iter([0, 1, 1, 20])
    monkeypatch.setattr(ai_review.time, "monotonic", lambda: next(ticks))
