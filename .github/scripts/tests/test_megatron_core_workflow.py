...
WORKFLOW = Path(__file__).resolve().parents[2] / "workflows/test-megatron-core.yml"


class MegatronCoreWorkflowTests(unittest.TestCase):
    def setUp(self):
        temporary = tempfile.TemporaryDirectory(prefix="megatron-core-workflow-")
        self.addCleanup(temporary.cleanup)
        self.root = Path(temporary.name)
        self.job = yaml.safe_load(WORKFLOW.read_text())["jobs"]["test-megatron-core"]
    ...
    def prepare_pages(self):
        ...
        self.page.write_text("Megatron-Core pip install wheel\n" + "details\n" * 524288)
        self.env.update(DOWNLOAD_URL=self.page.as_uri(), HOMEPAGE_URL=self.page.as_uri(),
                        OFFICIAL_DOCS=self.page.as_uri())
        doc = self.root / "content/linux/opensource_packages/megatron-core.md"
    ...
    def test_matching_partial_responses_do_not_hide_transfer_failures(self):
        self.prepare_pages()
        self.stub("curl", 'printf "Megatron-Core pip install wheel\\n"\nexit "$CURL_EXIT"\n')
        for step in ("test3",):
